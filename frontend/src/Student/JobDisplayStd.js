...
import SideNavBar1 from "../recruiter/SideNavbar1";
import JobViewStd from "./JobViewStd";


export default function JobDisplayStd() {
...
const closeModal1 = ()=>{
  ...
  async function getAllapplicants() {
      const response = await axios.get(`http://localhost:8080/institute/interestedclgs/showjobtostudents/${student.instituteName}`);
  ...
  async function addCollege(currentJobId) {
    ...
    try{
      ...
      const data = {
        ...
        status:"Applied",
        clgName:student.instituteName

      }
      const response1 = await axios.post("http://localhost:8080/student/interestedstudents", data);
  ...
  return (
    <div>
      ...
      centered show={isModalOpened} onHide={closeModal}>
          <Modal.Body><JobViewStd id={ID}/></Modal.Body>
          <Modal.Footer>
