...
function Personainfo() {
  ...
  const loadJob = async () => {
    const result = await axios.get(`http://localhost:8080/student/${student.studentId}`);
            ...
            <tr>
              <td>
                ...
                <input
                  ...
                  className="form-control form-control-sm mb-3 inputform "
                  required="value"
                  onChange={(event) => setpercentage(((marksobt / totmarks) * 100).toFixed(2))}
                ></input>
                <label
                  className="form-label"
                  htmlFor="form3Example1cg"
